...
const FAQ = () => {
  ...
  const previousOpenIdRef = useRef(null);
  ...
  const handleToggle = (itemId) => {
    ...
    if (clickedIndex >= 0 && containerRef.current) {
      ...
      if (items[clickedIndex]) {
        clickedItemTopBefore = items[clickedIndex].getBoundingClientRect().top + scrollYBefore;
      }
    }
    ...
    if (!wasOpen && previousOpenId && clickedIndex >= 0) {
      ...
      setTimeout(() => {
        if (containerRef.current) {
          const items = containerRef.current.querySelectorAll('[data-faq-item]');
          if (items[clickedIndex]) {
            const clickedItemTopAfter = items[clickedIndex].getBoundingClientRect().top + window.scrollY;
